...
const defaultThresholds = Object.freeze({
  schemaVersion: 1,
  minimumApprovedRows: 1124,
  minimumPriceCzk: 10,
  maximumPriceCzk: 50000,
  maximumRelativePriceChange: 1,
  suspiciousMultiplicativeFactor: 10,
  maximumChangedManagedRatio: 0.05,
  maximumSaleClearRatio: 0.02,
});
const {
...
const { MATCH_METHODS, validateAutomationBaseline } = require('./superzoo-automation-baseline');
...
function buildSuperZooManagedPriceCandidate(options = {}) {
  ...
  const scopeSha256 = automationBaselineSha256;
  ...
  if (automationBaseline) {
    ...
    addBlocker(groups, 'missing_automation_baseline');
  }
  ...
  const managedEntries = approvedBaselineEntries.filter(entry => entry?.dailyEligibility === 'exact_safe').map(entry => ({
    catalogSourceIdentity: entry.sourceIdentity,
    catalogPacking: entry.packing,
    rawSourceIdentity: entry.rawSourceIdentity || entry.sourceIdentity,
    rawPacking: entry.rawPacking || entry.packing,
    matchMethod: entry.matchMethod,
    productId: entry.productId,
    offerIdentity: { kind: 'product-partner', partner: entry.partner },
  }));
  const managedUrls = new Set(managedEntries.map(entry => entry?.rawSourceIdentity).filter(Boolean));
  ...
  for (const managed of managedEntries) {
    if (!MATCH_METHODS.has(managed?.matchMethod)) addBlocker(groups, 'invalid_managed_match_method', managed?.productId || '(missing)');
    if (typeof managed?.catalogSourceIdentity !== 'string' || !canonicalSourceUrl(managed.catalogSourceIdentity, effectiveConfig)
        || typeof managed?.rawSourceIdentity !== 'string' || !canonicalSourceUrl(managed.rawSourceIdentity, effectiveConfig)) {
    ...
    if (target.sourceUrl !== managed.catalogSourceIdentity) addBlocker(groups, 'managed_source_mapping_mismatch', managed?.productId);
    ...
    const expectedCatalogSourcePacking = sourcePackingKey(managed.catalogSourceIdentity, managed.catalogPacking);
    if (catalogBySourcePacking.length !== 1 || targetSourcePacking !== expectedCatalogSourcePacking) addBlocker(groups, 'ambiguous_managed_source_mapping', managed.catalogSourceIdentity);
    const rawSourcePacking = sourcePackingKey(managed.rawSourceIdentity, managed.rawPacking);
    const observedProducts = rawSourcePacking ? (rawBySourcePacking.get(rawSourcePacking) || []) : (rawBySource.get(managed.rawSourceIdentity) || []);
    ...
    if (observedProducts.length === 0 || (observedProducts.length > 1 && !equivalentProducts)) {
      addBlocker(groups, observedProducts.length === 0 ? 'managed_coverage_missing' : 'duplicate_source_identity', managed.rawSourceIdentity);
      continue;
    }
  ...
  const candidate = {
    ...
    partner: PARTNER,
    catalogSha256,
    scopeType: 'automation-baseline-exact-safe-approved',
    scopeSha256,
    reviewOnly: true,
  ...
  const report = {
    ...
    scopeType: 'automation-baseline-exact-safe-approved',
    scopeSha256,
    approvedTotal: approvedBaselineEntries.length,
    unresolvedApproved: approvedBaselineEntries.filter(entry => entry?.dailyEligibility === 'unresolved').length,
...
function main(argv = process.argv.slice(2)) {
  ...
  if (args['managed-set'] || args['expected-managed-set-sha256']) throw new Error('Legacy --managed-set inputs are not supported; use --automation-baseline.');
  const automationBaselinePath = requiredArg(args, 'automation-baseline');
  ...
  const automationBaseline = readJsonFile(automationBaselinePath);

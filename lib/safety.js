...
function approvedOutputRoots() {
  ...
  if (process.env.SUPERZOO_REVIEW_OUTPUT_ROOT) addTemporaryRoot(process.env.SUPERZOO_REVIEW_OUTPUT_ROOT, 'SUPERZOO_REVIEW_OUTPUT_ROOT');
  return roots;
}
...
function mainFoodScopeDecision(product, config) {
  const animalType = String(product.animalType || '').trim();
  const rules = config?.sourcePolicy?.mainFoodScopeRules || [];
  for (const rule of rules) {
    if (!rule.animalTypes?.includes(animalType)) continue;
    const fields = rule.fields || [];
    const values = fields.map(field => field === 'canonicalUrl'
      ? canonicalizeProductUrl(product.url || product.targetUrl || product.canonicalUrl, config)
      : String(product[field] || ''));
    const normalized = normalizeText(values.join(' '));
    for (const pattern of rule.patterns || []) {
      if (!new RegExp(pattern, 'iu').test(normalized)) continue;
      return { reason: 'main_food_scope', ruleId: rule.ruleId, reasonCode: rule.reasonCode, evidence: `${fields.join(', ')} matched ${pattern}`, sourceIdentity: product.sourceIdentity || null, canonicalIdentity: product.canonicalIdentity || null, sourceProductId: product.sourceProductId || product.sourceId || null, canonicalUrl: canonicalizeProductUrl(product.url || product.targetUrl || product.canonicalUrl, config) };
    }
  }
  return null;
}

function exclusionDecision(product, config, options = {}) {
  if (options.mainFoodScope && options.scopeBeforeLegacy) {
    const scopeDecision = mainFoodScopeDecision(product, config);
    if (scopeDecision) return scopeDecision;
  }
  if (product.id && config.catalogExclusionContract.legacyExcludedProductIds.includes(product.id)) return { reason: 'legacy_product_id', ruleId: null, reasonCode: null, evidence: 'product ID is in the legacy exclusion contract', sourceIdentity: product.sourceIdentity || null, canonicalIdentity: product.canonicalIdentity || null, sourceProductId: product.sourceProductId || product.sourceId || null, canonicalUrl: null };
  const rawUrl = product.url || product.targetUrl || product.canonicalUrl;
  if (rawUrl) {
    try {
      const canonicalUrl = canonicalizeProductUrl(rawUrl, config);
      ...
      if (excludedUrls.has(canonicalUrl)) return { reason: 'stable_source_url', ruleId: null, reasonCode: null, evidence: 'canonical URL is in the stable exclusion contract', sourceIdentity: product.sourceIdentity || null, canonicalIdentity: product.canonicalIdentity || null, sourceProductId: product.sourceProductId || product.sourceId || null, canonicalUrl };
    } catch {
      // URL validity is reported by the validator separately.
    }
  }
  const name = String(product.name || '');
  if (config.sourcePolicy.forbiddenNamePatterns.some(pattern => new RegExp(pattern, 'iu').test(name))) return { reason: 'forbidden_name', ruleId: null, reasonCode: null, evidence: 'name matched existing forbidden-name policy', sourceIdentity: product.sourceIdentity || null, canonicalIdentity: product.canonicalIdentity || null, sourceProductId: product.sourceProductId || product.sourceId || null, canonicalUrl: null };
  const category = String(product.category || '');
  if (config.sourcePolicy.forbiddenCategoryPatterns.some(pattern => new RegExp(pattern, 'iu').test(category))) return { reason: 'forbidden_category', ruleId: null, reasonCode: null, evidence: 'category matched existing forbidden-category policy', sourceIdentity: product.sourceIdentity || null, canonicalIdentity: product.canonicalIdentity || null, sourceProductId: product.sourceProductId || product.sourceId || null, canonicalUrl: null };
  return options.mainFoodScope ? mainFoodScopeDecision(product, config) : null;
}

function exclusionReason(product, config) {
  return exclusionDecision(product, config)?.reason || null;
}
...
module.exports = {
  ...
  canonicalizeProductUrl, countByCategory, exclusionDecision, exclusionReason, inferSize, isOutOfScope, loadConfig, mainFoodScopeDecision,

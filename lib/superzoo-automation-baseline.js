...
const CONTRACT = 'superzoo-automation-baseline-v1';
...
function indexRaw(raw, config, groups) {
  ...
  return { byIdentity, bySource, unresolved };
}

function makeApprovedEntry(product, offer, sourceIdentity, rawIndex) {
  const packing = packingEvidence(product);
  const key = identityKey(sourceIdentity, packing);
...
function buildAutomationBaseline(options = {}) {
  ...
  for (const product of Array.isArray(catalog) ? catalog : []) {
    ...
    const entry = makeApprovedEntry(product, offers[0], sourceIdentity, rawIndex);
  ...
  const baseline = {
    ...
    partner: PARTNER,
    source: SOURCE,
    identityContract: 'canonical-source-url-plus-packing-v1',
    evidence: {
...
function validateAutomationBaseline(baseline, expected = {}) {
  ...
  if (baseline?.identityContract !== 'canonical-source-url-plus-packing-v1') add('invalid_identity_contract');
...
function buildWeeklyDiff({ baseline, raw, config = loadConfig() }) {
  ...
  const identityChanges = approved.filter(entry => !rawIndex.byIdentity.has(entry.identityKey) && rawIndex.bySource.has(entry.sourceIdentity)).map(entry => ({
    productId: entry.productId,
    sourceIdentity: entry.sourceIdentity,
